...
document.addEventListener("DOMContentLoaded", () => {
    // Inicialización de Modales con validación
    ...
    const saleSearch = document.getElementById("saleSearchProduct");
    if(saleSearch) saleSearch.addEventListener("input", filterSaleResults);
    ...
    const historySearch = document.getElementById("searchSales");
    if(historySearch) historySearch.addEventListener("input", filterSalesHistory);
...
function filterSaleResults() {
    ...
    matches.forEach(p => {
        ...
        btn.innerHTML = `<div><div class="fw-bold">${p.name}</div><small>Stock: ${p.currentStock}</small></div><span>$${(p.finalSalesPrice || 0).toFixed(2)}</span>`;
...
async function handleSaleSubmit(e) {
    ...
    if(res.ok) {
        ...
        await loadProducts(); await loadSales(); await updateBalance();
    } else {
        const msg = await res.text(); Swal.fire("Error", msg || "Stock insuficiente", "error");
    }
}
...
function renderTable() {
    ...
    allProducts.forEach(p => {
        ...
        table.innerHTML += `
            <tr class="${rowClass}">
                <td class="ps-4 text-start"><div class="fw-bold">${p.name}</div><small class="text-muted">#${p.id}</small></td>
                ...
                <td class="text-end pe-4">
                    <button class="btn-action me-1" onclick='editProduct(${JSON.stringify(p)})'><i class="bi bi-pencil-fill"></i></button>
...
async function handleProductFormSubmit(e) {
    ...
    if(res.ok) {
        myModal.hide(); await loadProducts(); await updateBalance();
        ...
        sales.reverse().forEach(s => {
            ...
            table.innerHTML += `
                <tr>
                    ...
                    <td class="text-end pe-4">
                        <button class="btn btn-sm btn-outline-danger" onclick="askDelete(${s.id}, 'sale')"><i class="bi bi-trash"></i></button>
                    </td>
                </tr>`;
        });
    } catch (e) { console.error(e); }
...
async function handlePurchaseSubmit(e) {
    ...
    if(res.ok) {
        Swal.fire("OK", "Compra registrada", "success");
        e.target.reset(); await loadProducts(); await updateBalance();
    }
}
...
function askDelete(id, type) {
    ...
    document.getElementById("confirmText").innerText = type === 'product' ? '¿Borrar producto?' : '¿Anular venta? El stock regresará.';
    confirmModal.show();
}
...
async function executeDelete() {
    ...
    if(res.ok) {
        ...
        // REVERSIÓN DE STOCK: Recargamos productos SIEMPRE al eliminar
        await loadProducts();
        await loadSales();
        await updateBalance();
    }
