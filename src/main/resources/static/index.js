 ...
 * Desarrollado por: Walter (Software Architect)
 */

const BellaAfrodita = (() => {
    // 1. ESTADO Y CONFIGURACIÓN PRIVADA
    const API_URL = "/api";
    ...
    const setupEventListeners = () => {
        ...
        // Cálculos en tiempo real (Modal de Producto)
    ...
    async function updateBalance() {
        try {
            ...
            // 2. Recaudación con lógica de colores
            ...
            if(recapElem) {
                ...
                if (availableRecap < 0) {
                    recapElem.classList.remove("text-primary");
                    recapElem.classList.add("text-danger");
                } else {
                    recapElem.classList.remove("text-danger");
                    recapElem.classList.add("text-primary");
                }
            }
            ...
            // Nombre del mes
            const monthElem = document.getElementById("month-name");
            if(monthElem) {
                monthElem.innerText = new Date().toLocaleDateString('es-ES', { month: 'long' });
            }
        ...
        sales.reverse().forEach(s => {
            ...
            table.innerHTML += `
                <tr>
                    ...
                    <td><div class="fw-bold">${s.product ? s.product.name : 'Eliminado'} ${badgeMayor}</div></td>
                    <td>${s.quantity}</td>
                    ...
                    <td class="text-end pe-4">
                        <button class="btn btn-sm btn-outline-danger" onclick="BellaAfrodita.askDelete(${s.id}, 'sale')">
        ...
        purchases.reverse().forEach(p => {
            table.innerHTML += `
                <tr>
                    ...
                    <td class="text-end pe-4">
                        <button class="btn btn-sm btn-outline-danger" onclick="BellaAfrodita.askDelete(${p.id}, 'purchase')">
                            <i class="bi bi-trash"></i>
                        </button>
                    </td>
                </tr>`;
    ...
    async function handleSaleSubmit(e) {
        ...
        if(res.ok) {
            ...
            await loadProducts(); await updateBalance();
            toggleHistorial('ventas');
        } else {
            const msg = await res.text();
            Swal.fire("Error", msg || "Verifica el stock", "error");
        }
    }
        ...
        allProducts.forEach(p => {
            ...
            table.innerHTML += `
                <tr class="${rowClass}">
                    ...
                    <td class="text-end pe-4">
                        <button class="btn-action me-1" onclick='BellaAfrodita.editProduct(${JSON.stringify(p)})'>
                            <i class="bi bi-pencil-fill text-primary"></i>
                        </button>
                        <button class="btn-action text-danger" onclick="BellaAfrodita.askDelete(${p.id}, 'product')">
    ...
    async function handlePurchaseSubmit(e) {
        ...
        if(res.ok) {
            Swal.fire("Éxito", "Mercadería sumada al stock", "success");
            e.target.reset();
            await loadProducts(); await updateBalance();
    ...
    async function handleProductFormSubmit(e) {
        ...
        if(res.ok) {
            myModal.hide();
            await loadProducts(); await updateBalance();
            Swal.fire("Listo", "Producto guardado", "success");
        }
    }
    ...
    function showSection(id) {
        ...
        // Colapsar navbar en móviles al hacer clic
    ...
    function askDelete(id, type) {
        ...
        let texto = "¿Estás seguro?";
        if(type === 'sale') texto = "¿Anular venta? El stock regresará.";
        if(type === 'purchase') texto = "¿Anular compra? El stock se restará.";
    ...
    async function executeDelete() {
        let path = deleteTarget.type === 'sale' ? 'sales' : (deleteTarget.type === 'purchase' ? 'supply-orders' : 'products');
    ...
    // EXPOSICIÓN DE MÉTODOS PÚBLICOS
    return {
        init,
        showSection,
        openCreateModal,
...
// DISPARADOR
document.addEventListener("DOMContentLoaded", BellaAfrodita.init);